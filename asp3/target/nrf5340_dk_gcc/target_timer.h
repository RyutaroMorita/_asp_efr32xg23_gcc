...
#define TOPPERS_TIMER_BASE NRF5340_TIMER0_BASE

/*
 * タイマ割込みハンドラ登録のための定数
 */
#define INTNO_TIMER  (NRF5340_TIMER0_IRQn + 16) /* 割込み番号 */
...
{
    sil_wrw_mem(NRF5340_TIEMR_TASKS_CAPTURE(TOPPERS_TIMER_BASE, 1), 1);
    return sil_rew_mem(NRF5340_TIMER_CC(TOPPERS_TIMER_BASE, 1));
}
...
Inline void target_hrt_set_event(HRTCNT hrtcnt)
{
    ...
    sil_wrw_mem(NRF5340_TIMER_CC(TOPPERS_TIMER_BASE, 0), current + hrtcnt);
